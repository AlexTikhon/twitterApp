...
- **Apollo as server state:** React does not keep a second post array. Cursor pages merge through `fetchMore`; mutations update normalized entities or refetch the active feed when list reconciliation is safer.
- **Realtime:** Socket.IO uses the same short-lived JWT as HTTP. Create events refetch active feed queries, updates write the normalized `Post`, and deletes evict it. The policy is intentionally explicit rather than hidden in complex cache rules.
